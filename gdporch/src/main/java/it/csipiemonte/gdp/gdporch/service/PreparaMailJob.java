...
import it.csipiemonte.gdp.gdporch.dto.OrchPreparaMAILRequest;
...
public class PreparaMailJob {
    ...
    public MailComposition preparaMail(OrchPreparaMAILRequest body) {
        MailComposition output = new MailComposition();

        // --- 1. CERCO IL DESTINATARIO ---
        // Vado a prendermi la mail dell'utente partendo dal log
        String email = logRepository.findEmailByLogId(body.getIdLog());
        ...
        String nomeTestata = logRepository.findNomeTestataByLogId(body.getIdLog());
        String dataEdizioneStr = "";

        // Se non è un invio "storico" (ST), cerco la data dell'edizione
        if (body.getTipoMail() != null && !body.getTipoMail().toString().startsWith("ST")) {
            LocalDate dataEd = logRepository.findDataEdizioneByLogEdizioneId(body.getIdLogEdizione());
            if (dataEd != null) {
        ...
        GdpMail gdpMail = mailRepository.findById(body.getTipoMail().toString());
