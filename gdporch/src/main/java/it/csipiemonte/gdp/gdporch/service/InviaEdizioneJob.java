...
public class InviaEdizioneJob {
    ...
    private void processTask(GdpCodaCaricamento task, ChannelSftp channel) {
        ...
        try {
            ...
            try (InputStream is = channel.get(task.sftpPath);
                ...
                failTask(task, GdpMessage.F10_DAM_TRANSMISSION_FAILED, null, e.getMessage());
                return;
            }
            ...
            try {
                ...
                failTask(task, GdpMessage.F10_DAM_TRANSMISSION_FAILED, null, "API Exception: " + e.getMessage());
                return;
            }
            ...
            if (response != null && "SUBMITTED".equalsIgnoreCase(response.getStatus())) {
                ...
                failTask(task, GdpMessage.F10_DAM_TRANSMISSION_FAILED, response.getJobId(), "API FAILED, jobId: " + response.getJobId());
            } else {
                failTask(task, GdpMessage.F10_DAM_TRANSMISSION_FAILED, null, "Unexpected API status");
            }
    ...
    private void successTask(GdpCodaCaricamento task, String jobId, ChannelSftp channel) {
        ...
        if (logEdizione != null) {
            ...
            logEdizione.stato = StatoDamEdizione.SUBMITTED;
    ...
    private void failTask(GdpCodaCaricamento task, GdpMessage msgEnum, String jobId, String errorDesc) {
        ...
        GdpLogEdizione logEdizione = logEdizioneRepository.findById(task.fkGdpLogEdizione);
        if (logEdizione != null) {
            logEdizione.stato = StatoDamEdizione.FAILED;
            String detail = "F10: " + errorDesc;
            logEdizione.descrizione = logEdizione.descrizione == null ? detail : logEdizione.descrizione + " | " + detail;
            if (jobId != null) {
                logEdizione.jobId = jobId;
            }
